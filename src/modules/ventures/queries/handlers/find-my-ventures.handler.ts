...
export class FindMyVenturesHandler implements IQueryHandler<FindMyVentures, [Venture[], number]> {
  private readonly logger = new Logger(FindMyVenturesHandler.name);

  constructor(@InjectRepository(Venture) private readonly repository: Repository<Venture>) {}

  async execute(query: FindMyVentures): Promise<[Venture[], number]> {
    try {
      ...
      this.logger.error(
        `Find my ventures failed ownerId="${query.ownerId}" params="${JSON.stringify(query.params)}": ${error instanceof Error ? error.message : String(error)}`
      );
